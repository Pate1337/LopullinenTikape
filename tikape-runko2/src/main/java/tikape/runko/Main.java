...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        get("/rekisteroidy2", (req, res) -> {
            HashMap map = new HashMap<>();
            
            return new ModelAndView(map, "rekisteroidy2");
        }, new ThymeleafTemplateEngine());
        ...
        post("/luotunnus", (req, res) -> {
            ...
            Kayttaja kayttaja = KayttajaDao.findByUsernameAndPassword(tunnus, salasana2);
            if (kayttaja == null) {
                int kayttajaID = KayttajaDao.viimeisinKayttaja();
                KayttajaDao.delete(kayttajaID);
                res.redirect("/rekisteroidy2");
            }
            res.redirect("/");
            return "";
